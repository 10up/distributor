	...
	$( window ).load(
		 ...
		 */
			function doSuccess(results) {
				...
				_.each(
					results.internal, function(result, connectionId) {
				...
				_.each(
					results.external, function(result, connectionId) {
						if ('fail' === result.status) {
							error = true;
						} else {
		 ...
		 */
			$( distributorPushWrapper ).on(
				'click', '.add-connection', function(event) {
		 ...
		 */
			$( distributorPushWrapper ).on(
				'click', '.remove-connection', function(event) {
