...
function save_post( $post_id ) {
	...
	if ( empty( $_POST['dt_external_connection_pull_allowed_roles'] ) ) {
		delete_post_meta( $post_id, 'dt_external_connection_pull_allowed_roles' );
	} else {
		update_post_meta( $post_id, 'dt_external_connection_pull_allowed_roles', array_map( 'sanitize_text_field', $_POST['dt_external_connection_pull_allowed_roles'] ) );
	}
...
function meta_box_external_connection_details( $post ) {
	...
	$push_allowed_roles = get_post_meta( $post->ID, 'dt_external_connection_allowed_roles', true );
	if ( empty( $push_allowed_roles ) ) {
		$push_allowed_roles = array( 'administrator', 'editor' );
	} else {
		$push_allowed_roles[] = 'administrator';
	}

	$pull_allowed_roles = get_post_meta( $post->ID, 'dt_external_connection_pull_allowed_roles', true );
	if ( empty( $pull_allowed_roles ) ) {
		$pull_allowed_roles = array( 'administrator', 'editor' );
	} else {
		$pull_allowed_roles[] = 'administrator';
	}
	?>
	<?php
	...
	<fieldset class="dt-roles-allowed push-allowed hide-until-authed">
		...
		foreach ( $editable_roles as $role => $details ) {
			...
			<input class="dt-role-checkbox" name="dt_external_connection_allowed_roles[]" id="dt-role-<?php echo esc_attr( $role ); ?>" type="checkbox" <?php checked( true, in_array( $role, $push_allowed_roles, true ) ); ?> value="<?php echo esc_attr( $role ); ?>"> <label for="dt-role-<?php echo esc_attr( $role ); ?>"><?php echo esc_html( $name ); ?></label><br>
	...
	<fieldset class="dt-roles-allowed pull-allowed hide-until-authed">
		<legend><?php esc_html_e( 'Roles Allowed to Pull', 'distributor' ); ?></legend>
		...
		foreach ( $editable_roles as $role => $details ) {
			...
			<input class="dt-role-checkbox" name="dt_external_connection_pull_allowed_roles[]" id="dt-role-<?php echo esc_attr( $role ); ?>" type="checkbox" <?php checked( true, in_array( $role, $pull_allowed_roles, true ) ); ?> value="<?php echo esc_attr( $role ); ?>"> <label for="dt-role-<?php echo esc_attr( $role ); ?>"><?php echo esc_html( $name ); ?></label><br>
			<?php
		}
		?>
		<span class="description" style="margin-top: 20px;"><?php esc_html_e( 'Select the roles of users on this site that will be allowed to pull/push content to this connection. Keep in mind that pulling/pushing will use the permissions of the user credentials provided for this connection.', 'distributor' ); ?></span>
	</fieldset>
